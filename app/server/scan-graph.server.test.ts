...
import { createFridgeImage } from "./images.server";
import { PromptName } from "./prompts/registry.server";
import { adjudicateLocations } from "./scan/services/location-adjudication.server";
import {
  reconcileLocations,
  resolveDetectionZone,
} from "./scan/services/location-reconciliation.server";
import { detectInventory } from "./scan/services/inventory-detection.server";
import { reconcileInventory } from "./scan/services/inventory-reconciliation.server";
import { mapZones } from "./scan/services/zone-map.server";
...
} from "./scan/schemas/scan-result";
import { validateImagesNode } from "./scan/index.server";
...
describe("detectInventory", () => {
  ...
  it("returns invalid detection validation for invalid raw detections", async () => {
    const image = createFridgeImage({
      dataUrl: createJpegDataUrl(),
      originalName: null,
      storageLocation: "fridge",
    ...
    await expect(
      ...
      detectionModelRawOutput: {
	      ...
	      detectionValidation: {
	        valid: false,
	        reason: expect.stringContaining("rawDetections.0.bbox.width"),
	      },
	    });
	  });
...
describe("location reconciliation", () => {
  ...
  it("resolves confident geometry matches", () => {
    const zone = {
      id: "zone-1",
      img: "image-1",
      type: "shelf",
    ...
    expect(resolveDetectionZone(detection, [zone])).toMatchObject({
      status: "matched",
  ...
  it("marks close candidate scores as ambiguous", () => {
    const zones = [
      {
        id: "zone-1",
        img: "image-1",
  ...
  it("sends low-scoring zone candidates to adjudication", () => {
    const zone = {
      id: "zone-1",
      img: "image-1",
      type: "shelf",
    ...
    expect(resolveDetectionZone(detection, [zone])).toEqual({
      status: "ambiguous",
      detectionId: "detection-1",
      candidates: [
        {
  ...
  it("marks detections as unmatched when no same-image zone exists", () => {
  ...
  it("returns ambiguous adjudication requests from scan state", async () => {
    const zones = [
      {
        id: "zone-1",
        img: "image-1",
...
describe("inventory reconciliation", () => {
  it("stores general recipe ingredients on reconciled inventory items", async () => {
    ...
    await expect(
	  ...
	  it("stores null depthBackRatio for unmatched detections", async () => {
	    await expect(
	      reconcileInventory({
	        fridgeId: "fridge-1",
	        imageIds: ["image-1"],

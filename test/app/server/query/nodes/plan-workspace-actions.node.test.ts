...
import { createPlanWorkspaceActionsNode } from "../../../../../app/server/query/nodes/plan-workspace-actions.node";
...
describe("workspace action planner", () => {
  it("skips visual action planning after a selected inventory label is applied", async () => {
    const assertionState = state() as { context: Record<string, unknown> };
    assertionState.context = {
    ...
    const result = await createPlanWorkspaceActionsNode()(assertionState as never);

    expect(result.context?.workspaceActions).toEqual([]);
    expect("workspaceActionError" in (result.context ?? {})).toBe(false);
  });

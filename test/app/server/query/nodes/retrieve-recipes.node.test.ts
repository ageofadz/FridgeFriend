...
} from "../../../../../app/server/query/nodes/retrieve-recipes.node";
...
describe("retrieve recipes node", () => {
  it("preserves a Chroma retrieval failure instead of substituting a SQL-only result", async () => {
    const node = createRetrieveRecipesNode({
      searchRecipeCandidates: async () => {
        throw new Error("Chroma collection is unavailable");
      },
      getRecipeCandidatesByTags: () => [],
      getRecipeCandidatesByIngredients: () => [],
    });

    await expect(node(baseState())).rejects.toThrow("Chroma collection is unavailable");
  });
  ...
  it("does not introduce a related semantic query during retrieval", async () => {
    const queries: string[] = [];
    ...
    const node = createRetrieveRecipesNode({
      loadInventoryForImage: () => inventoryWithIngredients(["egg", "lemon", "yogurt", "whipped topping"]),
      searchRecipeCandidates: async ({ query }) => {
        queries.push(query);
        return query === "chocolate dessert recipes"
          ? [{ recipeId: "dessert-with-egg", semanticScore: 0.8 }]
          : [];
      },
      getRecipeCandidatesByTags: () => [],
      getRecipeCandidatesByIngredients: () => [],
      getRecipesByIds: () => [dessertRecipe],
    });

    const result = await node({
      ...stateWithInventory(),
      query: "Show easy dessert recipes from these ingredients.",
      recipeSearch: {
        ...baseState().recipeSearch!,
        semanticQuery: "easy chocolate dessert recipes",
        intent: { specific: true, relatedSemanticQuery: "chocolate dessert recipes" },
        useAvailableIngredients: false,
        excludedIngredients: [],
        dietaryRestrictions: [],
        maxMinutes: null,
    ...
    expect(queries).toEqual(["easy chocolate dessert recipes"]);
    expect(result.context.recipeRetrieval).toMatchObject({
      noMatches: true,
      audit: { terminalReason: "vector_empty" },
      recipes: [],
    });
  });

  it("keeps a named recipe theme intent-first when the inventory includes unrelated produce", async () => {
    const queries: string[] = [];
    let searchedIngredients: string[] = [];
    const snackRecipe: Recipe = {
      ...chickenRecipe,
      id: "snack-bites",
    ...
    const unrelatedRecipe: Recipe = {
      ...chickenRecipe,
      id: "vegetable-savoury-bake",
      name: "Vegetable Savoury Bake",
      ingredients: [
        { rawName: "bell pepper", canonicalName: "bell pepper" },
        { rawName: "asparagus", canonicalName: "asparagus" },
    ...
    const node = createRetrieveRecipesNode({
      loadInventoryForImage: () => inventoryWithRecipeItems([
        { name: "egg carton", subcat: "egg", cat: "eggs", pack: "carton" },
        { name: "yogurt cup", subcat: "yogurt", cat: "dairy", pack: "container" },
        { name: "milk carton", subcat: "milk", cat: "dairy", pack: "carton" },
        { name: "blueberries", subcat: "blueberry", cat: "produce", pack: "container" },
        { name: "cherries", subcat: "cherry", cat: "produce", pack: "container" },
        { name: "apple", subcat: "apple", cat: "produce", pack: "loose" },
        { name: "bell pepper", subcat: "bell pepper", cat: "produce", pack: "loose" },
        { name: "asparagus", subcat: "asparagus", cat: "produce", pack: "loose" },
      ]),
      searchRecipeCandidates: async ({ query }) => {
        queries.push(query);
        return query === "easy snack recipes"
          ? [{ recipeId: "snack-bites", semanticScore: 0.85 }]
          : [];
      },
      getRecipeCandidatesByTags: () => [],
      getRecipeCandidatesByIngredients: (input) => {
        searchedIngredients = input.ingredients;
        return [{
          recipeId: "vegetable-savoury-bake",
          matchedIngredients: ["egg", "bell pepper", "asparagus"],
          ingredientScore: 1,
        }];
      },
      getRecipesByIds: () => [snackRecipe, unrelatedRecipe],
    });

    const result = await node({
      ...stateWithInventory(),
      query: "Show easy snack recipes from these ingredients.",
      recipeSearch: {
        ...baseState().recipeSearch!,
    ...
    expect(queries).toEqual(["easy snack recipes"]);
    expect(searchedIngredients).toEqual(["egg", "yogurt", "milk", "blueberry", "cherry", "apple", "bell pepper", "asparagu"]);
    expect(result.context.recipeRetrieval).toMatchObject({
      noMatches: false,
      selectedIntentTier: "primary",
      recipes: [{ id: "snack-bites", matchedIngredients: ["egg"], intentTier: "primary" }],
    });
    expect(result.tournamentCandidates.map((recipe) => recipe.id)).toEqual(["snack-bites"]);
  });

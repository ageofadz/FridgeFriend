...
import {
  ...
  loadFoodComRecipes,
} from "../../../../app/server/recipes/foodcom.server";
import { indexFoodComRecipes } from "../../../../app/server/recipes/indexing.server";
import { buildRecipeRetrievalText } from "../../../../app/server/recipes/normalization";
import {
...
} from "../../../../app/server/recipes/vector-store.server";
...
describe("Food.com recipe corpus", () => {
  ...
  it("parses serialized Food.com fields, aggregates ratings, normalizes ingredients, and selects deterministically", async () => {
    ...
    expect(buildRecipeRetrievalText(recipes[1])).toBe(
  ...
  it("pushes numeric hard constraints down into the Chroma where clause", async () => {
    ...
    const collection = {
      handle: {
        ...
        query: async (input: { where: unknown }) => {
          ...
          return { rows: () => [[{ metadata: { recipeId: "a" }, distance: 0 }]] };
        },
      },
    };
    const embeddings = {
    ...
    await expect(searchRecipeCandidates({
      ...
      embeddings,
      getCollection: async () => collection as never,
    })).resolves.toEqual([{ recipeId: "a", semanticScore: 1 }]);
    expect(wheres).toEqual([{
      $and: [
        ...
        { proteinDailyValue: { $gte: 25 } },
      ],
    }]);

    await searchRecipeCandidates({ query: "any dinner", limit: 30 }, {
      embeddings,
      getCollection: async () => collection as never,
    });
    expect(wheres[1]).toEqual({ documentType: "recipe" });
  });
});

...
import {
  ...
  fridges,
  users,
} from "../../../../app/server/db/schema.server";
import {
  DEFAULT_FRIDGE_ID,
  DEFAULT_USER_ID,
  bootstrapSqlite,
  withDatabase,
} from "../../../../app/server/sqlite.server";
import {
...
} from "../../../../app/server/memory/repository.server";
import type { MemoryCandidate } from "../../../../app/server/memory/schemas";
...
describe("memory repository", () => {
  ...
  it("checks semantic-memory availability by user and fridge scope", () => {
    const profile = {
      userId: DEFAULT_USER_ID,
      fridgeId: DEFAULT_FRIDGE_ID,
    };

    expect(hasActiveSemanticMemories(profile)).toBe(false);
    ...
    persist({
      ...
      action: "upsert",
      category: "household",
      content: "Dinner is usually served at seven.",
      explicit: true,
    });

    expect(hasActiveSemanticMemories(profile)).toBe(true);
  });
